...
#test 1
../pipex_bonus "$input" "cat" "grep name" "grep -E a$" "wc -l" "$output_b"1 && cat "$input" | grep name | grep -E a$ | wc -l > "$path_bon"1;
...
sleep 0.3
#test 2
../pipex_bonus "$input" "cat" "grep -i a" "grep -E e$" "$output_b"2 && cat "$input" | grep -i a | grep -E e$  > "$path_bon"2;
...
sleep 0.3
#test 3
../pipex_bonus "$input" "cat" "grep -v a" "grep -E s$" "$output_b"3 && cat "$input" | grep -v a | grep -E s$ > "$path_bon"3;
...
sleep 0.3
#test 4
../pipex_bonus "$input" "cat" "grep -i s" "sort" "head -n 3" "uniq" "tail -5" "wc -l" "$output_b"4 && cat "$input" | grep -i s | sort | head -n 3 | uniq |tail -5 | wc -l > "$path_bon"4;
...
sleep 0.3
#test 5
../pipex_bonus "$input" "cat" "grep -i s" "head -n 3" "sort"  "uniq" "tail -5" "$output_b"5 && cat "$input" | grep -i s | head -n 3 | sort | uniq |tail -5  > "$path_bon"5;
...
make bonus -C ../ 1> /dev/null
#test 2
( (echo "hello"; echo "emim"; echo "nice"; echo "test"; echo "my life"; echo "EOF") | ../pipex_bonus here_doc EOF "cat" "grep -i e" "$here_doc"1) > /dev/null ;
...
sleep 0.3
#test 2
( (echo "hello" ; echo "im not"; echo "life"; echo "EOF") | ../pipex_bonus here_doc EOF "cat" "wc -l" "$here_doc"2) > /dev/null ;
